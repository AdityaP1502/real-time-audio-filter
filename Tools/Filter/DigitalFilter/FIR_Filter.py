...
class FIR_Filter(Filter):
  ...
  def __filter_overlap_save(self, xn_0, xn_1):
    ...
    assert type(xn_0).__name__ == "list", "input must have type list. Not {}".format(type(xn_0).__name__)
    assert type(xn_1).__name__ == "list", "input must have type list. Not {}".format(type(xn_1).__name__)
    ...
    assert len(xm_0) == self.sample_length, "the length of the block must eqaul to sample length {}. Received {}".format(self.sample_length, len(xm_0))
    assert len(xm_1) == self.sample_length, "the length of the block must eqaul to sample length {}. Received {}".format(self.sample_length, len(xm_1))
    
    y0 = convfft_overlap_save(xm_0, self.hn)[self.N:] # discard M - 1 element
    y1 = convfft_overlap_save(xm_1, self.hn)[self.N:] # discard M - 1 element
    
    # append data from the output buffer to y0
    y = self.output_buffer + y0[0 : self.sample_length - (len(self.output_buffer))]
    
    # add data from the next output chunk
    if len(y) < self.sample_length:
      y = y + y1[0:self.sample_length - (len(y))]
    
    assert len(y) == self.sample_length, "Each output chunk must have {} length. Received {}".format(self.sample_length, len(y))  
    res.append(y)
    buffer_length = len(self.output_buffer) + len(y0) + len(y1) - self.sample_length
